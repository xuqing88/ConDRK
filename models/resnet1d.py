...
class ResNet1d(nn.Sequential):
    ...
    def __init__(self, block, layers, kernel_size=3, num_classes=2, input_channels=3, inplanes=64,
                 fix_feature_dim=True, kernel_size_stem = None, stride_stem=2, pooling_stem=True,
                 stride=2,lin_ftrs_head=None, ps_head=0.5, bn_final_head=False,
                 bn_head=True, act_head="relu", concat_pooling=True,lam=0.01):
        ...
        # Orignial Structure
        # layers_tmp.append(AdaptiveConcatPool1d(1))
        # layers_tmp.append(nn.Flatten())
        # layers_tmp.append(nn.BatchNorm1d(inplanes * block.expansion *2))
        # layers_tmp.append(nn.Dropout(p=0.25, inplace=False))
        # layers_tmp.append(nn.Linear(inplanes * block.expansion*2, inplanes * block.expansion))
        # layers_tmp.append(nn.ReLU(inplace=True))
        # layers_tmp.append(nn.BatchNorm1d(inplanes * block.expansion))
        # layers_tmp.append(nn.Dropout(p=0.5, inplace=False))
        # layers_tmp.append(nn.Linear(inplanes * block.expansion, num_classes))


        # layers_tmp.append(AdaptiveConcatPool1d(1))
        # layers_tmp.append(nn.Flatten())
        # layers_tmp.append(nn.Linear(inplanes * block.expansion, inplanes * block.expansion // 2))
        # # layers_tmp.append(nn.Linear(inplanes * block.expansion*2, inplanes * block.expansion//2))
        # layers_tmp.append(nn.BatchNorm1d(inplanes * block.expansion//2))
        # layers_tmp.append(nn.ReLU(inplace=True))
        # layers_tmp.append(nn.Dropout(p=0.25, inplace=False))
        # layers_tmp.append(nn.Linear(inplanes * block.expansion//2, num_classes))

        #
        # head = create_head1d((inplanes if fix_feature_dim else (2**len(layers)*inplanes)) * block.expansion,
        #                      nc=num_classes, lin_ftrs=lin_ftrs_head, ps=ps_head,
        #                      bn_final=bn_final_head, bn=bn_head,
        ...
        # self.flatten = nn.Sequential(AdaptiveConcatPool1d(1),
        #                           nn.Flatten()
        #                              )
        self.head_fc = nn.Sequential(
                                    AdaptiveConcatPool1d(1),
                                    nn.Flatten(),
                                  nn.Linear(inplanes * block.expansion*2 , inplanes * block.expansion // 2),
                                  nn.BatchNorm1d(inplanes * block.expansion//2),
                                  nn.ReLU(inplace=True),
                                  nn.Dropout(p=0.25, inplace=False),
                                  nn.Linear(inplanes * block.expansion//2, num_classes))

        # CSRA
        self.head_csra = nn.Conv2d(inplanes * block.expansion, num_classes, 1, bias=False)
...
class AdaptiveConcatPool1d(nn.Module):
    ...
    def forward(self, x):
        # return self.ap(x) + 0.01 * self.mp(x)
...
class CNN(nn.Module):
    def __init__(self, input_channels):
        ...
        mid_channels = 32
        final_out_channels = mid_channels*2
        dropout = 0.5
        num_classes = 6

        self.conv_block1 = nn.Sequential(
            nn.Conv1d(self.input_channels, mid_channels, kernel_size=5,
                      stride=1, bias=False, padding=(5 // 2)),
        ...
        self.conv_block3 = nn.Sequential(
            nn.Conv1d(mid_channels * 2, final_out_channels, kernel_size=8, stride=1, bias=False,
                      padding=4),
