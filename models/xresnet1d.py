...
class XResNet1d(nn.Sequential):
    ...
    def __init__(self, block, expansion, layers, p=0.0, input_channels=3, num_classes=1000, stem_szs=(32,32,64),kernel_size=5,kernel_size_stem=5,
        ...
        # head = create_head1d(block_szs[-1]*expansion, nc=num_classes,
        #                      lin_ftrs=lin_ftrs_head, ps=ps_head, bn_final=bn_final_head, bn=bn_head, act=act_head, concat_pooling=concat_pooling)
        # super().__init__(
        ...
        blocks.append(AdaptiveConcatPool1d(1))
        blocks.append(nn.Flatten())
        blocks.append(nn.Linear(block_szs[-1]*expansion*2, block_szs[-1]*expansion//2))
        blocks.append(nn.BatchNorm1d(block_szs[-1]*expansion//2))
        blocks.append(nn.ReLU(inplace=True))
        blocks.append(nn.Dropout(p=0.25, inplace=False))
        blocks.append(nn.Linear(block_szs[-1]*expansion//2, num_classes))
