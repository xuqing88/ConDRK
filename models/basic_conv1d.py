...
# from models.resnet1d import AdaptiveConcatPool1d
...
class basic_conv1d(nn.Sequential):
    ...
    def __init__(self, filters=[128,128,128,128],kernel_size=3, stride=2, dilation=1, pool=0, pool_stride=1,
                 squeeze_excite_reduction=0, num_classes=2, input_channels=8, act="relu", bn=True,
                 headless=False,split_first_layer=False,drop_p=0.,lin_ftrs_head=None, ps_head=0.5, bn_final_head=False,
        ...
        if(headless is True):
            ...
            # layers.append(nn.Linear(filters[-1] * 2, filters[-1] // 2))
            # layers.append(nn.BatchNorm1d(filters[-1] // 2))
            # layers.append(nn.ReLU(inplace=True))
            # layers.append(nn.Dropout(p=0.25, inplace=False))
            # layers.append(nn.Linear(filters[-1] // 2, num_classes))

            self.head_fc = nn.Sequential(AdaptiveConcatPool1d(1),
                                      nn.Flatten(),
                                      nn.Linear(filters[-1] * 2 , filters[-1] // 2),
                                      nn.BatchNorm1d(filters[-1] // 2),
                                      nn.ReLU(inplace=True),
                                      nn.Dropout(p=0.25, inplace=False),
                                      nn.Linear(filters[-1] // 2, num_classes))

        # layers.append(head)
            # CSRA
            self.head_csra = nn.Conv2d(filters[-1], num_classes, 1, bias=False)
            self.lam = 0.1
